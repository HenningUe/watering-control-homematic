...
from offsim.simulator import sensortrigger
...
def get_simulate_func(run_scenario_func, do_plot):
    ...
    def simulate_inner(ccu):
        ...
        if do_plot:
            from offsim.simulator import plotter
...
class _MyDevices(object):
    ...
    def trigger_watering_gesamt(cls):
        sensortrigger.trigger_event_but_short(cls.schalter_wozi_1.switch_top_right)

    @classmethod
    def trigger_watering_sued(cls):
        sensortrigger.trigger_event_but_short(cls.schalter_wozi_1.switch_middle_left)

    @classmethod
    def trigger_watering_west(cls):
        sensortrigger.trigger_event_but_short(cls.schalter_wozi_1.switch_middle_right)
...
class FlowerPots(object):
    ...
    def wasserstand_sensor_event_handler(flowerpot_name, event_type):
        ...
        if is_open_state is not None:
            sensortrigger.trigger_event_shutter_contact(channel, is_open_state)

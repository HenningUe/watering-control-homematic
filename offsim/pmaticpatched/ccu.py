...
from . import api, events, utils  # @UnusedImport
from .entities import Devices, Device, Rooms, Room  # @UnusedImport
...
#         args = list(args)
#         args = args[1:]
#         newo =  object.__new__(cls, *args, **kwargs)
#         newo.__init__(**kwargs)
#         return newo
#         #return super(CCU, cls).__new__(cls)
    ...
    def events(self):
        ...
        if not self._events:
            self._events = events.EventListener(self)
        return self._events
    ...
    def signal_strengths(self):
        if self._rssi == None:
            self._rssi = api.SignalStrength(self.api)
        return self._rssi
